...
function update(tree) {
    ...
    for (const [projectName, config] of projects.entries()) {
        ...
        const appConfigPath = (0, devkit_1.joinPathFragments)(config.root, 'app.json');
        const appConfigTsPath = (0, devkit_1.joinPathFragments)(config.root, 'app.config.ts');
        const appConfigJsPath = (0, devkit_1.joinPathFragments)(config.root, 'app.config.js');
...
function updateJsonSplashScreenConfig(tree, configPath, projectName) {
    try {
        (0, devkit_1.updateJson)(tree, configPath, (config) => {
            if (config.expo?.splash) {
                ...
                const existingPluginIndex = config.expo.plugins.findIndex((plugin) => plugin === 'expo-splash-screen' ||
                ...
                devkit_1.logger.info(`Migrated splash screen configuration to plugin format for ${projectName}`);
            }
            return config;
        });
    }
    catch (error) {
        devkit_1.logger.warn(`Failed to update splash screen configuration for ${projectName}: ${error}`);
    }
}
function updateJsConfigSplashScreenConfig(tree, configPath, projectName) {
    try {
        ...
        if (content.includes('splash:') && !content.includes('plugins:')) {
            ...
            if (match) {
                ...
                const imageMatch = splashConfig.match(/image\s*:\s*['"`]([^'"`]+)['"`]/);
                const imageUrlMatch = splashConfig.match(/imageUrl\s*:\s*['"`]([^'"`]+)['"`]/);
                const backgroundColorMatch = splashConfig.match(/backgroundColor\s*:\s*['"`]([^'"`]+)['"`]/);
                const resizeModeMatch = splashConfig.match(/resizeMode\s*:\s*['"`]([^'"`]+)['"`]/);
                ...
                if (backgroundColorMatch) {
                    pluginConfig.push(`      backgroundColor: '${backgroundColorMatch[1]}'`);
                }
                if (resizeModeMatch) {
...
${pluginConfig.join(',\n')}
                    ...
                    updatedContent = updatedContent.replace(splashRegex, pluginConfigString);
...
`;
        ...
        devkit_1.logger.warn(`Failed to update JS/TS config for ${projectName}: ${error}`);
    }
}
