...
function postTargetTransformer(migrationLogs, processOptions) {
    return (target, tree, projectDetails, inferredTargetConfiguration) => {
        if (target.options) {
            processOptions(tree, target.options, projectDetails.projectName, projectDetails.root, migrationLogs);
        }
        if (target.configurations) {
            for (const configurationName in target.configurations) {
                const configuration = target.configurations[configurationName];
                processOptions(tree, configuration, projectDetails.projectName, projectDetails.root, migrationLogs);
            }
